...
test("the merged profile control opens a responsive account dashboard", async () => {
  ...
  assert.match(css, /\.account-header-button[^}]*grid-template-columns: 64px[^}]*gap: 12px[^}]*padding: 12px 14px/);
  assert.match(css, /\.account-header-badge[^}]*position: relative[^}]*width: 58px[^}]*height: 58px[^}]*translate\(-23px, -28px\)/);
