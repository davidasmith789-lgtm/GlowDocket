...
export async function sendExternalReminderTest(profile) { const connection = await connectExternalPush(profile, { requestPermission: true }); if (!connection.device) return connection; await api("test", { profileInstallationId: connection.device.profileInstallationId }, connection.device.token); return { status: "active", device: connection.device }; }
