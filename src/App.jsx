...
const getVisibleViewportWidth = () => {
  const windowWidth = Number(window.innerWidth) || 0;
  const documentWidth = Number(document.documentElement?.clientWidth) || windowWidth;
  return Math.min(windowWidth, documentWidth);
};
...
const getAvailableWorkspaceWidth = () => Math.max(960, getVisibleViewportWidth() - 32);
...
function WorkspaceWidget({
  ...
  const availableBodyHeight = Math.max(1, Number(instance.height) - labelHeight - widgetBodyPadding);
  ...
  const resizeStart = (event, edges = { right: true, bottom: true }) => {
    ...
    const startHeight = collapsed ? labelHeight : Number(instance.height);
    const minimumHeight = collapsed ? labelHeight : minimumExpandedHeight;
  ...
  const labelResizeStart = (event) => {
    ...
    const header = widget?.querySelector(".workspace-widget-header");
    const canvas = widget?.closest(".workspace-widget-canvas");
    if (!widget || !header) return;
    try { handle.setPointerCapture?.(activePointerId); } catch { /* Window listeners remain as a fallback. */ }
    const canvasScale = Number(canvas?.dataset.workspaceScale) || 1;
    const startY = event.clientY;
    ...
    const stop = (stopEvent) => {
      if (stopEvent?.pointerId !== undefined && stopEvent.pointerId !== activePointerId) return;
      widget.classList.remove("is-label-resizing");
      window.removeEventListener("pointermove", move);
      window.removeEventListener("pointerup", stop);
      window.removeEventListener("pointercancel", stop);
      try { if (handle.hasPointerCapture?.(activePointerId)) handle.releasePointerCapture(activePointerId); } catch { /* Capture may already be released. */ }
      onLabelResize(nextLabelHeight);
    };
  ...
  return (
    ...
    >
      <header className="workspace-widget-header double-click-collapse-header" onDoubleClick={(event) => toggleFromHeaderDoubleClick(event, onToggle)}>
        ...
        {!locked && <button type="button" className="widget-label-resize-handle" onPointerDown={labelResizeStart} aria-label={`Resize ${title} label height`} title="Drag to change label height" />}
      </header>
      {!collapsed && (
...
function App() {
  ...
  useEffect(() => {
    const handleResize = () => setAppViewportWidth(getVisibleViewportWidth());
  ...
  const dedicatedGoogleCalendarId = googleCalendarState.preferences?.destination_kind === "dedicated" ? googleCalendarState.preferences?.dedicated_calendar_id : "";
  ...
  const workloadPeriodLabels = { today: "Today", week: "This week", month: "This month", custom: "Custom period", all: "All remaining" };
  ...
  const renderWidgetContent = (instanceOrType) => {
    ...
    if (type === "stat-workload") return (
      <div className={`portable-stat workload-stat${workloadPeriod === "custom" ? " is-custom-period" : ""}`}>
        <label><span>Time period</span><select style={{ "--workload-select-width": `${Math.max(8, workloadPeriodLabels[workloadPeriod].length + 3)}ch` }} value={workloadPeriod} onChange={(event) => handleAddFieldSettingChange("workloadPeriod", event.target.value)}><option value="today">Today</option><option value="week">This week</option><option value="month">This month</option><option value="custom">Custom period</option><option value="all">All remaining</option></select></label>
        ...
        <strong className={!workloadSummary.invalid && workloadSummary.knownMinutes <= 0 ? "is-no-estimates" : ""}>{workloadSummary.invalid ? "Choose dates" : workloadTimeLabel}</strong>
  ...
  const renderWorkspaceInstance = (instance) => (
    <WorkspaceWidget
      ...
      onResize={(width, height, canvasWidth, x = instance.x, y = instance.y, resizeState = {}) => updateWidgetInstance(instance.id, resizeState.collapsed
        ? { width, labelHeight: height, collapsedHeight: height, x, y, xRatio: canvasWidth > 0 ? x / canvasWidth : instance.xRatio }
        : { width, height, expandedHeight: height, x, y, xRatio: canvasWidth > 0 ? x / canvasWidth : instance.xRatio }, { canvasWidth })}
      onLabelResize={(height) => updateWidgetInstance(instance.id, { labelHeight: height, collapsedHeight: height })}
  ...
  const renderMobileSummaryAssignments = () => mobileSummaryTasks.length === 0 ? (
    ...
    <ul className="task-list mobile-summary-task-list">
      {mobileSummaryTasks.map((task) => {
        ...
        const summaryOpen = !isMobileUi || level >= MOBILE_TASK_LEVELS.summary;
        const detailsOpen = isMobileUi ? level >= MOBILE_TASK_LEVELS.details : expandedTaskId === task.id;
        return <li key={task.id} className={`task-card task-card-status-${status}${isMobileUi ? ` mobile-task-card mobile-task-level-${level}` : ""}${status === "inProgress" ? " in-progress-task-card" : ""}${getTaskDueBucket(task).startsWith("Overdue") ? " is-overdue" : ""}${summaryOpen ? " expanded" : ""}${detailsOpen ? " assignment-summary-details-open" : ""}`} role="button" tabIndex="0" aria-expanded={detailsOpen} onClick={() => toggleTaskExpansion(task.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); toggleTaskExpansion(task.id); } }}><div><div className="task-title-row"><strong className="task-title-text">{task.title}</strong><span className="task-course-pill" style={{ backgroundColor: getCourseColor(getTaskCourseOrCategory(task)), color: getTextColorForCourse(getTaskCourseOrCategory(task)) }}>{getTaskCourseOrCategory(task)}</span>{isMobileUi ? <span className="mobile-workflow-pill">{getWorkflowLabel(status)}</span> : status === "inProgress" ? <span className="in-progress-status-pill">In Progress</span> : null}</div>{summaryOpen && <div className="task-details">{formatTaskDetails(task)}</div>}{getTaskDueBucket(task).startsWith("Overdue") || summaryOpen ? renderAssignmentCountdown(task) : null}{summaryOpen && renderSubtaskProgressLine(task)}</div>{summaryOpen && renderTaskActionButtons(task, status)}{detailsOpen && <div className="task-notes-panel" onClick={(event) => event.stopPropagation()}>{renderExpandedTaskDetails(task, `mobile-summary-notes-${task.id}`)}</div>}</li>;
      })}
    </ul>
  );
                ...
                {settingsSection === "calendar" && (<>
                  ...
                  {googleCalendarPreviewEnabled ? <SettingsCard title="Google Calendar" description="See selected Google calendars here and keep GlowDocket school items synchronized." className="settings-section-wide google-calendar-settings" collapsible={false}>
                    {!googleCalendarState.connected ? <>
                      ...
                      <fieldset className="google-calendar-options"><legend>Import from</legend><div>{googleCalendarChoices.map((calendar) => { const isAutomatic = calendar.id === dedicatedGoogleCalendarId; return <label className="google-calendar-compact-option" key={calendar.id}><input type="checkbox" disabled={isAutomatic} checked={isAutomatic || googleCalendarState.selectedCalendars?.some((item) => item.calendar_id === calendar.id && item.selected_for_import)} onChange={(event) => { const selected = new Set((googleCalendarState.selectedCalendars || []).filter((item) => item.selected_for_import).map((item) => item.calendar_id)); if (event.target.checked) selected.add(calendar.id); else selected.delete(calendar.id); handleGoogleSetting({ import_calendar_ids: [...selected] }); }} /><span>{calendar.summary}{isAutomatic && <small>Automatic</small>}</span></label>; })}</div></fieldset>
