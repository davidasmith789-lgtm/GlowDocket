...
export function validateReminder(raw, now = Date.now()) {
  ...
  const allowed = new Set(["taskId", "occurrenceKey", "title", "course", "preferredName", "deadline", "timeZone", "leadMinutes", "revision"]);
  ...
  const taskId = String(raw.taskId || "").trim(); const title = String(raw.title || "").trim(); const course = String(raw.course || "").trim(); const preferredName = String(raw.preferredName || "").trim();
  ...
  if (!/^[A-Za-z0-9_.:-]{1,120}$/.test(taskId) || !title || title.length > 160 || course.length > 100 || preferredName.length > 60 || /[\r\n<>]/.test(preferredName) || !revision || revision.length > 120) throw new ReminderError("invalid_reminder", "The reminder fields are invalid.");
  ...
  return { taskId, occurrenceKey, title, course, preferredName, deadline: deadlineDate.toISOString(), timeZone, leadMinutes, reminderSendTime: new Date(reminderSendTime).toISOString(), revision };
}

function mapRow(row) {
  if (!row) return null;
  return { id: row.id, profileInstallationId: row.profile_installation_id, taskId: row.task_id, occurrenceKey: row.occurrence_key, subscriptionId: row.onesignal_subscription_id, messageId: row.onesignal_message_id, title: row.assignment_title, course: row.course || "", preferredName: row.preferred_name || "", deadline: row.deadline, reminderSendTime: row.reminder_send_time, timeZone: row.timezone, leadMinutes: row.lead_time_minutes, revision: row.local_revision, status: row.scheduling_status, lastError: row.last_error };
}
function toRow(record) {
  return { profile_installation_id: record.profileInstallationId, task_id: record.taskId, occurrence_key: record.occurrenceKey, onesignal_subscription_id: record.subscriptionId, onesignal_message_id: record.messageId || null, assignment_title: record.title, course: record.course || null, preferred_name: record.preferredName || null, deadline: record.deadline, reminder_send_time: record.reminderSendTime, timezone: record.timeZone, lead_time_minutes: record.leadMinutes, local_revision: record.revision, scheduling_status: record.status, last_error: record.lastError || null, updated_at: new Date().toISOString() };
}
...
export function createReminderService({ registry, oneSignal, secret, now = () => Date.now(), externalEnabled = true }) {
  ...
  return {
    ...
    test: async (input, token) => { if (!externalEnabled) throw new ReminderError("external_push_disabled", "External push testing is currently paused.", 503); const device = await authenticate(input.profileInstallationId, token); const preferredName = String(input.preferredName || "").trim(); if (preferredName.length > 60 || /[\r\n<>]/.test(preferredName)) throw new ReminderError("invalid_reminder", "The preferred name is invalid."); return { messageId: await oneSignal.test(device.subscriptionId, idempotencyKeyFor(device.profileInstallationId, "test", String(Math.floor(now() / 60000))), preferredName) }; },
